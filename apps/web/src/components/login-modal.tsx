...
export function LoginModal({ onClose }: { onClose: () => void }) {
  ...
  const onSubmitCredentials = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      setError(err instanceof AuthApiError ? err.message : "no se pudo iniciar sesión");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const onSubmitTwoFactor = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      setError(err instanceof AuthApiError ? err.message : "código inválido");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      >
        ...
        {step === "credentials" ? (
          ...
          <form onSubmit={onSubmitTwoFactor} className="flex flex-col gap-3">
            <p className="font-mono text-xs text-cci-muted">código de la app autenticadora</p>
            <Field label="código">
              <input
                type="text"
                inputMode="numeric"
            ...
            {error && <p className="font-mono text-xs text-cci-danger">{error}</p>}
            <SubmitButton submitting={submitting}>verificar</SubmitButton>
          </form>
        )}
      </div>
    </div>
